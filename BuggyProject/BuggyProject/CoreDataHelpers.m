...
+ (NSArray *)arrayForFetchRequestWithName: (NSString *)name {
	...
	if (error) {
		fetchResult = [NSArray array];
		NSLog(@"%@", [error localizedFailureReason]);
	}
	
    NSSortDescriptor *srtByOwnerName = [[NSSortDescriptor alloc] initWithKey:@"owner.ownerName"
                                                                   ascending:YES];
    NSSortDescriptor *srtByModelName = [[NSSortDescriptor alloc] initWithKey:@"modelName"
                                                                   ascending:NO];
    
    NSMutableArray *srtedResults = [NSMutableArray arrayWithArray:fetchResult];
    [srtedResults sortUsingDescriptors:@[srtByOwnerName, srtByModelName]];
    
	return [NSArray arrayWithArray:srtedResults];
}
